...
public abstract class IterativeRecommender extends Recommender {
	...
	protected DenseMatrix P, PC, last_User_factor;

	// factorized item-factor matrix
	protected DenseMatrix Q, QC, last_Item_factor;

	// user biases
	protected DenseVector userBias, cUserBias, last_UB;
	// item biases
	protected DenseVector itemBias, cItemBias, last_IB;
	...
	protected boolean isConverged(int iter) {
		...
		try {
			...
			if (iter==1) {
				...
				Logs.info(iter+"\tWhole:"+result);
				
				if(coldUserThreshold > 0){
					measures = isRankingPred ? super.coldStartEvalRankings() : super.evalRatings();
				}
				String coldResult = getEvalInfo(measures);
				Logs.info(iter+"\tCold:"+coldResult);
				
			}
			else if ( iter%numPrintIters == (numPrintIters - 1) ) {
				measures = isRankingPred ? super.evalRankings() : super.evalRatings();
				String result = getEvalInfo(measures);
				Logs.info(iter+"\tWhole:"+result);
