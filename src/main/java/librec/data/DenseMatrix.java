...
public class DenseMatrix implements Serializable {
	...
	public void init(double range) {
		...
		for (int i = 0; i < numRows; i++)
			for (int j = 0; j < numColumns; j++)
				data[i][j] = Randoms.uniform(-0.02, range);
	}
	...
	public static double rowSquareMult(DenseMatrix m, int mrow, DenseMatrix n, int nrow) {
		assert m.numColumns == n.numColumns;

		double res = 0;
		for (int j = 0, k = m.numColumns; j < k; j++)
			res += m.get(mrow, j) * Math.pow(n.get(nrow, j),2);
	 ...
	 * @param DenseMatrix rightMatrix
	 * @return 用于三个矩阵乘，Yt*C*Y.其中 C为对角矩阵，且有些值可以为空。
	 * 此版本可以加速计算
	 *@create_time：2015年1月18日下午8:09:36
	 *@modifie_time：2015年1月18日 下午8:09:36
	 ...
	 * @return 此方法用于计算 Yt*Cu*Pu
	 *       其中Cu为对角矩阵，Pu为稀疏向量。通过此方法可以加速计算
