...
public class Sims {
	...
	/**
	 * calculate common negibor
	 */
	public static double cn(Set<Integer> a, Set<Integer> b) {
		int count=0;
		for (Integer entry : b) {
			if (a.contains(entry)) {
				count++;
	 ...
	 * @return Jaccard's coefficient
	 */
	public static double jaccard(int[] uItems, int[] vItems) {
		int common = 0, all = 0;
		Set<Integer> items = new HashSet<>();
		for (int value : uItems) {
			items.add(value);
		}
		for (int value  : vItems) {
			items.add(value);
		}
		
		all = items.size();
		common = uItems.length + vItems.length - all;
		return (common + 0.0) / all;
