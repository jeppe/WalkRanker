...
//		for (int i = 0; i < p; i++) {
//			for (int j = 0; j < p; j++) {
//				z.set(i, j, Distribution.normalRandom(0, 1));
//			}
//		}
//		
//		SparseVector y = new SparseVector(p);
//		for (int i = 0; i < p; i++) {
//			y.set(i, gammaRandom((df - (i+1))/2, 2));
//		}
//		
...
//					zz.set(k, z.get(k, j));
//				}
//				B.setValue(j, j, y.get(j) + zz.innerProduct(zz));
//			}
//
...
//						zkj.set(k, z.get(k, j));
//					}
//					B.set(i, j, z.get(i, j) * Math.sqrt(y.get(i)) + zki.innerProduct(zkj));
